...
from load_utils import load_audio_text_image_video, extract_fbank
...
class SenseVoiceSmall(nn.Module):
    ...
    def inference(
        ...
        **kwargs,
        ...
        print(f"1 event_emo_query: {event_emo_query}")
        input_query = torch.cat((language_query, event_emo_query), dim=1)
        speech = torch.cat((input_query, speech), dim=1)
        speech_lengths += 3
        print(f"1 speech.shape: {speech}")
        print(f"1 speech_lengths: {speech_lengths}")
        # Encoder
        encoder_out, encoder_out_lens = self.encoder(speech, speech_lengths)
        if isinstance(encoder_out, tuple):
            encoder_out = encoder_out[0]
        print(f"1 encoder_out.shape: {encoder_out.shape}")
        print(f"1 encoder_out_lens: {encoder_out_lens}")
        # c. Passed the encoder result and the beam search
        ctc_logits = self.ctc.log_softmax(encoder_out)
        print(f"1 ctc_logits.shape: {ctc_logits.shape}")
        results = []
        ...
        for i in range(b):
            ...
            yseq = x.argmax(dim=-1)
            print(f"1 最大索引序列: {yseq}")
            yseq = torch.unique_consecutive(yseq, dim=-1)
            print(f"1 最大索引序列移除重复元素: {yseq}")
            ibest_writer = None
            ...
            print(f"1 生成的token序列: {token_int}")
